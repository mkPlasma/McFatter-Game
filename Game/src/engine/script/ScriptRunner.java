...
public class ScriptRunner{
	...
	private void builtInFunction(int index, int lineNum){
		...
		if(dot){
			...
			while(!funcParams.isEmpty())
				tmp.add(funcParams.remove());
			
			// Add dot value
			funcParams.add(dotValue);
			dotValue = null;
		...
		if(funcParams.size() > 0){
			o1 = funcParams.remove();
			
			if(funcParams.size() > 0)
				o2 = funcParams.remove();
		}
		...
		int params = getBuiltInFunctionParameterCount(index);
