...
public class BulletInstruction extends Instruction{
	...
	public static final short
		...
		CONST_SPD_DIR =			SET_DIR | SET_SPD,
		CONST_ACCEL =			CONST_SPD_DIR | SET_ACCEL | SET_SPD_MIN_MAX,
		CONST_ANGVEL =			CONST_SPD_DIR | SET_ANGVEL,
		CONST_ANGVEL_ACCEL =	CONST_ACCEL | CONST_ANGVEL,
		
		// Same as above, except values are changed rather than set
		OFFSET_SPD_DIR =		CHANGE_VALUE | CONST_SPD_DIR,
	...
	public BulletInstruction(Bullet bullet, int time, int type, float[] args){
		...
		dir = type == SET_DIR || type == CHANGE_DIR ? args[0] : type == CONST_SPD_DIR || type == CONST_ACCEL || type == CONST_ANGVEL || type == CONST_ANGVEL_ACCEL || type == OFFSET_SPD_DIR || type == OFFSET_ACCEL || type == OFFSET_ANGVEL || type == OFFSET_ANGVEL_ACCEL  ? args[0] : 0;
		...
		spd = type == SET_SPD || type == CHANGE_SPD ? args[0] : type == CONST_SPD_DIR || type == CONST_ACCEL || type == OFFSET_SPD_DIR || type == OFFSET_ACCEL ? args[1] : type == CONST_ANGVEL || type == CONST_ANGVEL_ACCEL || type == OFFSET_ANGVEL || type == OFFSET_ANGVEL_ACCEL ? args[2] : 0;
